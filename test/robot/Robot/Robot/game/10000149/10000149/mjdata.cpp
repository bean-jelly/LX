...
return  card count
*/
int CMJHandCard::OnGetCardCount(unsigned char cardvalue)
{
...
{
	if (m_hand_length - count < 0 || IsCardValid(cardvalue) == false)
	{
		return -1;
	}
	if ((unsigned int)OnGetCardCount(cardvalue) < count)
	{
		return -1;
	}
...
{ 
	...
	{
		if (IsCardValid(i) && m_hand_length <= HAND_CARD_COUNT)
		{
...
return  card count
*/
int CMJOutCard::OnGetCardCount(unsigned char cardvalue)
{
	if (IsCardValid(cardvalue) == false)
	{
		return 0;
...
{
	if (m_length - count < 0 || IsCardValid(cardvalue) == false)
	{
		return -1;
	}
	if ((unsigned int)OnGetCardCount(cardvalue) < count)
...
void CMJOutCard::CardSort()
{
	...
	for (unsigned int i = 0; i < CARD_INDEX_NUM; i++)
	{
		if (IsCardValid(i) && m_length < MAX_OUT_CARD)
		{
			for (unsigned int s = 0; s < card_index[i]; s++)
			{
...
bool CMJWallCard::Swap(unsigned int first_pos, unsigned int second_pos)
{
	...
	if (first_pos >= m_wall_len || first_pos >= m_wall_len)
	{
		return false;
	}
...
//获取指定牌个数
int CMJFlowerCard::OnGetCardCount(unsigned char cardvalue)
{
	if (IsCardValid(cardvalue) == false)
	{
		return 0;
...
void CMJFlowerCard::CardSort()
{
	...
	for (unsigned int i = 0; i < CARD_INDEX_NUM; i++)
	{
		if (IsCardValid(i) && m_length <= MAX_FLOWER_NUM)
		{
			for (unsigned int s = 0; s < card_index[i]; s++)
			{
